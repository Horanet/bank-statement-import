...
class AccountBankStatementImportSheetParser(models.TransientModel):
    ...
    def parse(self, mapping, data_file, filename):
        ...
        balance_end, lines = self._parse_lines(mapping, data_file, currency_code)
        if not lines:
    ...
    def _parse_lines(self, mapping, data_file, currency_code):
        ...
        line_delta = mapping.header_line_number - 1
        balance_end = None
        if isinstance(csv_or_xlsx, tuple):
            header = [str(value) for value in csv_or_xlsx[1].row_values(0 + line_delta)]
            if mapping.balance_end_cell:
                balance_end = csv_or_xlsx[1].cell_value(
                    int(mapping.balance_end_cell[1]) - 1,
                    ord(mapping.balance_end_cell[0])-65,
                )
                balance_end = self._parse_decimal(balance_end, mapping)
        else:
        ...
        debit_amount_column = header.index(mapping.debit_amount_column) \
            if mapping.debit_amount_column else None
        credit_amount_column = header.index(mapping.credit_amount_column) \
            if mapping.credit_amount_column else None
        ...
        if isinstance(csv_or_xlsx, tuple):
            rows = range(1 + line_delta, csv_or_xlsx[1].nrows)
        else:
            rows = csv_or_xlsx
        ...
        for row in rows:
            if isinstance(csv_or_xlsx, tuple):
                ...
                # Si la première colonne de la ligne est vide, on passe à la ligne suivante
                if not sheet.cell_value(row, 0).strip():
                    continue
            ...
            # Il y a soit une colonne montant, soit une colonne débit et une colonne crédit
            if amount_column is not None:
                amount = values[amount_column]
            else:
                if values[debit_amount_column]:
                    amount = values[debit_amount_column]
                    debit_credit = mapping.debit_value
                else:
                    amount = values[credit_amount_column]
                    debit_credit = mapping.credit_value
